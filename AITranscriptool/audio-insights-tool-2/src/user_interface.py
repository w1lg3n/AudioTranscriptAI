...
from src.summarize_transcript_by_gpt import summarize_transcript
from src.helper import chunk_transcript, merge_keywords_dictionaries
...
def process_summary(transcript):
    """Handle summary of the transcription."""
    with st.spinner("Summarize the transcript..."):
        summary = summarize_transcript(transcript)
        summary = json.loads(summary)
        if summary:
            st.write("#### Summary:")
            with st.expander("Click to show transcript summary", expanded=False):
...
def handle_keyword_search(transcript):
    ...
    with st.spinner("Searching for keywords in the transcript..."):
        found_keywords_list = None
        for chunk in chunk_transcript(transcript=transcript, chunk_size=70):
            result = json.loads(searched_keywords(chunk))
            result = result["Detected Key Phrases"]
            if found_keywords_list:
                found_keywords_list = merge_keywords_dictionaries(found_keywords_list, result)
            else:
                found_keywords_list = result
        cols = st.columns(3)
        for i, values in enumerate(found_keywords_list.items()):
            if values[1]:
                with cols[i % 3]:
...
def handle_audio_file(audio_file, max_file_size_mb):
    ...
    if file_size_mb > max_file_size_mb:
        ...
        if process_clicked:
            ...
            if transcript:
                # Handle Summary, keyword search and sentiment analysis
                process_summary(transcript)
